...
fn format_offset_for_disasm(addr_type: Option<AddrType16>, disp_type: Option<DisplacementType>, offset: u64) -> String {
    ...
    if addr_type_str == None {
        ...
        format!("[{}+{} {:x}]", addr_type_str.unwrap(), disp_type_str.unwrap(), offset)
    }
}
...
impl Cpu {
    ...
    pub fn get_opcode_params_from_modrm<T: CpuBus>(
        ...
        match mode {
            0 => {
                ...
                match disp_type {
                    ...
                    Some(DisplacementType::Byte) => {
                        self.modrm_disp =
                            self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip)
                                as u64;
                        self.regs.rip = self.regs.rip.wrapping_add(1);
                    }
                    Some(DisplacementType::Word) => {
                        self.modrm_disp =
                        self.mem_read16(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip) as u64;
            ...
            1 => {
                ...
                let addr: u64;
                self.modrm_disp =
                    self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip) as u64;
    ...
    pub fn tick<T: CpuBus>(&mut self, bus: &mut T) {
        ...
        println!("opcode: {:x} cs: {:x} rip: {:x}", opcode, self.regs.segs[SegReg::CS as usize].selector, self.regs.rip);
        self.regs.rip = self.regs.rip.wrapping_add(1);

        match opcode {
            0x0a => {
                let modrm = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                let opcode_params = self.get_opcode_params_from_modrm(bus, modrm);
                if let Operand::Register(rm) = opcode_params.rm {
                    println!("or {}, {}", Into::<&'static str>::into(<u8 as Into<Reg8>>::into(opcode_params.reg)), Into::<&'static str>::into(<u8 as Into<Reg8>>::into(rm)));
                    ...
                    println!("or {}, {}:{}", Into::<&'static str>::into(<u8 as Into<Reg8>>::into(opcode_params.reg)), Into::<&'static str>::into(segment), format_offset_for_disasm(Cpu::get_addr_type_from_modrm16(modrm), Cpu::get_disp_type_from_modrm(modrm), self.modrm_disp));
            ...
            0x0c => {
                let imm = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("or al, {:x}", imm);
            ...
            0x0d => {
                if (self.use_32op_default == Use32OpFlags::Bits32 && !self.use_32op_prefix) || (self.use_32op_default == Use32OpFlags::Bits16 && self.use_32op_prefix) {
                    let imm = self.mem_read32(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                    ...
                    let imm = self.mem_read16(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                    self.regs.rip = self.regs.rip.wrapping_add(2);
                    println!("or ax, {:x}", imm);
            ...
            0x0f => {
                let opcode2 = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                ...
                match opcode2 {
                    0x01 => {
                        ...
                        match modrm & 0x38 {
                            0x20 => {
                                let opcode_params = self.get_opcode_params_from_modrm(bus, modrm);
                                if let Operand::Register(rm) = opcode_params.rm {
                                    println!("smsw {}", Into::<&'static str>::into(<u8 as Into<Reg16>>::into(rm)));
                                    self.regs.write16(rm.into(), self.regs.cr0 as u16);
                                }
                                else if let Operand::Address(segment, ea) = opcode_params.rm {
                                    println!("smsw {}:{}", Into::<&'static str>::into(segment), format_offset_for_disasm(Cpu::get_addr_type_from_modrm16(modrm), Cpu::get_disp_type_from_modrm(modrm), self.modrm_disp));
                                    self.mem_write16(bus, self.regs.segs[segment as usize].base + ea, self.regs.cr0 as u16);
                                }
                            },
                            _ => todo!(),
                        }
                    ...
                    0x20 => {
                        ...
                        match reg {
                            0 => {
                                if let Operand::Register(rm) = opcode_params.rm {
                                    println!("mov {}, cr0", Into::<&'static str>::into(<u8 as Into<Reg32>>::into(rm)));
                                    self.regs.write32(rm.into(), self.regs.cr0 as u32 | 0x7fffffe0u32);
                                }
                            },
                            _ => todo!(),
                        }
            ...
            0x12 => {
                ...
                let opcode_params = self.get_opcode_params_from_modrm(bus, modrm);
                if let Operand::Register(rm) = opcode_params.rm {
                    println!("adc {}, {}", Into::<&'static str>::into(<u8 as Into<Reg8>>::into(opcode_params.reg)), Into::<&'static str>::into(<u8 as Into<Reg8>>::into(rm)));
                    let rm = self.regs.read8(rm.into());
                    let carry = self.regs.getflag(Flags::Carry);
                    let reg = self.regs.read8(opcode_params.reg.into());
                    let result = self.regs.read8(opcode_params.reg.into()).wrapping_add(rm).wrapping_add(carry as u8);
                    self.regs.write8(opcode_params.reg.into(), result);
                    self.setznp8(result);
                    ...
                    println!("adc {}, {}:{}", Into::<&'static str>::into(<u8 as Into<Reg8>>::into(opcode_params.reg)), Into::<&'static str>::into(segment), format_offset_for_disasm(Cpu::get_addr_type_from_modrm16(modrm), Cpu::get_disp_type_from_modrm(modrm), self.modrm_disp));
                    let rm = self.mem_read8(bus, self.regs.segs[segment as usize].base + ea);
                    let carry = self.regs.getflag(Flags::Carry);
                    let reg = self.regs.read8(opcode_params.reg.into());
                    let result = self.regs.read8(opcode_params.reg.into()).wrapping_add(rm).wrapping_add(carry as u8);
            ...
            0x33 => {
                ...
                if (self.use_32op_default == Use32OpFlags::Bits32 && !self.use_32op_prefix) || (self.use_32op_default == Use32OpFlags::Bits16 && self.use_32op_prefix) {
                    if let Operand::Register(rm) = opcode_params.rm {
                        println!("xor {}, {}", Into::<&'static str>::into(<u8 as Into<Reg32>>::into(opcode_params.reg)), Into::<&'static str>::into(<u8 as Into<Reg32>>::into(rm)));
                        ...
                        println!("xor {}, {}:{}", Into::<&'static str>::into(<u8 as Into<Reg32>>::into(opcode_params.reg)), Into::<&'static str>::into(segment), format_offset_for_disasm(Cpu::get_addr_type_from_modrm16(modrm), Cpu::get_disp_type_from_modrm(modrm), self.modrm_disp));
                    ...
                    if let Operand::Register(rm) = opcode_params.rm {
                        println!("xor {}, {}", Into::<&'static str>::into(<u8 as Into<Reg16>>::into(opcode_params.reg)), Into::<&'static str>::into(<u8 as Into<Reg16>>::into(rm)));
                        ...
                        println!("xor {}, {}:{}", Into::<&'static str>::into(<u8 as Into<Reg16>>::into(opcode_params.reg)), Into::<&'static str>::into(segment), format_offset_for_disasm(Cpu::get_addr_type_from_modrm16(modrm), Cpu::get_disp_type_from_modrm(modrm), self.modrm_disp));
            ...
            0x70 => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("jo {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
                if self.regs.getflag(Flags::Overflow) == 1 {
                    self.regs.rip = (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16) as u64;
                }
            },
            0x71 => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("jno {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
            ...
            0x72 => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("jc {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
            ...
            0x73 => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("jnc {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
            ...
            0x74 => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("jz {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
            ...
            0x75 => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("jnz {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
            ...
            0x76 => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("jbe {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
            ...
            0x77 => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("jnbe {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
            ...
            0x78 => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("js {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
            ...
            0x79 => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("jns {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
            ...
            0x7a => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("jp {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
            ...
            0x7b => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("jnp {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
            ...
            0x7c => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("jl {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
            ...
            0x7d => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("jnl {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
                if self.regs.getflag(Flags::Sign) == self.regs.getflag(Flags::Overflow) {
                    self.regs.rip = (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16) as u64;
                }
            },
            0x7e => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("jle {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
                let check = self.regs.getflag(Flags::Overflow) == 1 || self.regs.getflag(Flags::Zero) == 1;
                let check_real;
                if check {
                    check_real = 1;
                }
            ...
            0x7f => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("jnle {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
                let check = self.regs.getflag(Flags::Overflow) == 1 && self.regs.getflag(Flags::Zero) == 0;
                let check_real;
                if check {
                    check_real = 1;
                }
            ...
            0x81 => {
                ...
                match modrm & 0x38 {
                    0x38 => {
                        if let Operand::Register(rm) = opcode_params.rm {
                            println!("cmp {}, {:x}", Into::<&'static str>::into(<u8 as Into<Reg16>>::into(rm)), imm);
                            let result = self.regs.read16(rm.into()).wrapping_sub(imm);
                            self.setznp16(result);
                
                            if ((result ^ imm) & (result ^ self.regs.read16(rm.into())) & 0x8000) == 0x8000 {
                                self.regs.setflag(Flags::Overflow, 1);
                            }
                            else {
                                self.regs.setflag(Flags::Overflow, 0);
                        ...
                        if let Operand::Address(segment, ea) = opcode_params.rm {
                            println!("cmp {}:{}, {:x}", Into::<&'static str>::into(segment), format_offset_for_disasm(Cpu::get_addr_type_from_modrm16(modrm), Cpu::get_disp_type_from_modrm(modrm), self.modrm_disp), imm);
                            let rm = self.mem_read16(bus, self.regs.segs[segment as usize].base + ea);
            ...
            0x8b => {
                ...
                if let Operand::Register(rm) = opcode_params.rm {
                    ...
                    println!("mov {}, {}", Into::<&'static str>::into(<u8 as Into<Reg16>>::into(reg)), Into::<&'static str>::into(<u8 as Into<Reg16>>::into(rm)));
                }
                else if let Operand::Address(segment, ea) = opcode_params.rm {
                    let rm = self.mem_read16(bus, self.regs.segs[segment as usize].base + ea);
                    self.regs.write16(reg.into(), rm);
                    println!("mov {}, {}:{}", Into::<&'static str>::into(<u8 as Into<Reg16>>::into(reg)), Into::<&'static str>::into(segment), format_offset_for_disasm(Cpu::get_addr_type_from_modrm16(modrm), Cpu::get_disp_type_from_modrm(modrm), self.modrm_disp));
                }
            },
            0x8c => {
                let modrm = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                ...
                if let Operand::Register(rm) = opcode_params.rm {
                    self.regs.write16(rm.into(), self.regs.segs[Into::<u8>::into(seg) as usize].selector);
                    println!("mov {}, {}", Into::<&'static str>::into(<u8 as Into<Reg16>>::into(rm)), Into::<&'static str>::into(<u8 as Into<SegReg>>::into(seg)));
                }
                else if let Operand::Address(segment, ea) = opcode_params.rm {
                    self.mem_write16(bus, self.regs.segs[segment as usize].base + ea, self.regs.segs[Into::<u8>::into(seg) as usize].selector);
                    println!("mov {}:{}, {}", Into::<&'static str>::into(segment), format_offset_for_disasm(Cpu::get_addr_type_from_modrm16(modrm), Cpu::get_disp_type_from_modrm(modrm), self.modrm_disp) ,Into::<&'static str>::into(<u8 as Into<SegReg>>::into(seg)));
                }
            }
            0x8e => {
                let modrm = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                ...
                let seg = opcode_params.reg;
                if let Operand::Register(rm) = opcode_params.rm {
                    self.regs.writeseg_realmode(seg.into(), self.regs.read16(rm.into()));
                    println!("mov {}, {}", Into::<&'static str>::into(<u8 as Into<SegReg>>::into(seg)), Into::<&'static str>::into(<u8 as Into<Reg16>>::into(rm)));
                }
                else if let Operand::Address(segment, ea) = opcode_params.rm {
                    let rm = self.mem_read16(bus, self.regs.segs[segment as usize].base + ea);
                    self.regs.writeseg_realmode(seg.into(), rm);
                    println!("mov {}, {}:{}", Into::<&'static str>::into(<u8 as Into<SegReg>>::into(seg)), Into::<&'static str>::into(segment), format_offset_for_disasm(Cpu::get_addr_type_from_modrm16(modrm), Cpu::get_disp_type_from_modrm(modrm), self.modrm_disp));
                }
            },
            0xa8 => {
                let imm = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
            ...
            0xe3 => {
                let offset = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                println!("jcxz {:x}", (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16));
                if self.regs.read16(Reg16::CX) == 0 {
                    self.regs.rip = (self.regs.rip as u16).wrapping_add(offset as i8 as i16 as u16) as u64;
                }
            },
            0xe4 => {
                let port = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
            ...
            0xe9 => {
                let offset = self.mem_read16(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(2);
                println!("jmp near {:x}", (self.regs.rip as u16).wrapping_add(offset));
                self.regs.rip = (self.regs.rip as u16).wrapping_add(offset) as u64;
            },
            0xea => {
                let newip = self.mem_read16(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(2);
                let newcs = self.mem_read16(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(2);
            ...
            0xf5 => {
                println!("cmc");
                self.regs.setflag(Flags::Carry, !self.regs.getflag(Flags::Carry) & 1);
            }
            0xf6 => {
                let modrm = self.mem_read8(bus, self.regs.segs[SegReg::CS as usize].base + self.regs.rip);
                self.regs.rip = self.regs.rip.wrapping_add(1);
                match modrm & 0x38 {
                    0x00 => {
                        ...
                        self.regs.rip = self.regs.rip.wrapping_add(1);
                        if let Operand::Register(rm) = opcode_params.rm {
                            println!("test {}, {:x}", Into::<&'static str>::into(<u8 as Into<Reg8>>::into(rm)), imm);
                            let result = self.regs.read8(rm.into()) & imm;
                            self.setznp8(result);
                        }
                        else if let Operand::Address(segment, ea) = opcode_params.rm {
                            println!("test {}:{}, {:x}", Into::<&'static str>::into(segment), format_offset_for_disasm(Cpu::get_addr_type_from_modrm16(modrm), Cpu::get_disp_type_from_modrm(modrm), self.modrm_disp), imm);
                            let rm = self.mem_read8(bus, self.regs.segs[segment as usize].base + ea);
                            let result = rm & imm;
                            self.setznp8(result);
                        }
